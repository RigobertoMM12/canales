...
import { CotizacionModelN } from './model/CotizacionModelN';
...
export class CcotizacionesComponent implements OnInit {
  ...
  config: any;
  colleccionCo:CotizacionModelN[]=[];
  collection = { count: 60, data: this.colleccionCo };
  ...
  loadData(): void {
    ...
    this.cotizacionService.getCotizacion().subscribe(
      resp => {
        if (resp.length > 0) {
          resp.forEach((element: CotizacionModelN) => {
  ...
  descripcionProduct(element:CotizacionModelN){
